 ...
 */
function fugu_theme_municipio_search_shortcode($atts) {
    ...
    // Get all municipios for initial population
    ...
    <div class="municipio-search-widget">
        <div class="search-step search-provincia">
            <label for="provincia-select"><?php _e('Selecciona Provincia:', 'fugu-theme'); ?></label>
            <select id="provincia-select" class="provincia-select">
                <option value=""><?php _e('-- Selecciona una provincia --', 'fugu-theme'); ?></option>
        ...
        <div class="search-step search-municipio" style="margin-top: 20px;">
            <label for="municipio-select"><?php _e('Selecciona Municipio:', 'fugu-theme'); ?></label>
            <select id="municipio-select" class="municipio-select">
                <option value=""><?php _e('-- Selecciona un municipio --', 'fugu-theme'); ?></option>
    ...
    jQuery(document).ready(function($) {
        ...
        // Cache original municipio options for filtering
        ...
        $provinciaSelect.select2({
            placeholder: '<?php _e('-- Selecciona una provincia --', 'fugu-theme'); ?>',
            allowClear: true,
            width: '100%',
        ...
        $municipioSelect.select2({
            placeholder: '<?php _e('-- Selecciona un municipio --', 'fugu-theme'); ?>',
            allowClear: true,
            width: '100%',
            dropdownCssClass: 'lenis-prevent-dropdown'
        });
        ...
        // Initial render with all municipios
        renderMunicipios('');

        // When provincia changes, filter municipios
...
add_shortcode('municipio_search', 'fugu_theme_municipio_search_shortcode');

/**
 * AJAX handler to get municipios by provincia
 */
function fugu_theme_get_municipios_by_provincia() {
...
add_action('wp_ajax_get_municipios_by_provincia', 'fugu_theme_get_municipios_by_provincia');
add_action('wp_ajax_nopriv_get_municipios_by_provincia', 'fugu_theme_get_municipios_by_provincia');
