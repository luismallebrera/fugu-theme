...
function fugu_theme_import_galgdr_municipios() {
    // Path to CSV file
    ...
    while (($data = fgetcsv($handle, 0, ';')) !== FALSE) {
        ...
        // Create or get provincia taxonomy term
        ...
        // Assign provincia to GDR (always update to ensure it's correct)
        ...
        foreach ($municipios as $municipio_name) {
            ...
            if (!$existing_municipio) {
                ...
                // Update existing municipio with GDR and Provincia
...
// Add admin menu to run import
add_action('admin_menu', 'fugu_theme_import_menu');
function fugu_theme_import_menu() {
    add_management_page(
        'Import GDR & Municipios',
        'Import GDR',
        'manage_options',
        'import-galgdr-municipios',
        'fugu_theme_import_page'
    );
}

function fugu_theme_import_page() {
    ?>
    <div class="wrap">
        ...
        if (isset($_POST['run_import']) && check_admin_referer('import_galgdr_nonce')) {
            $results = fugu_theme_import_galgdr_municipios();
