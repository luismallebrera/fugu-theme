...
Fugu Elementor is a lightweight and minimalist WordPress theme that was built specifically to work seamlessly with the Elementor site builder plugin. The theme is free, open-source, and designed for users who want a flexible, easy-to-use, and customizable website.
...
Fugu Elementor bundles the following third-party resources:
...
= 3.2.1 - 2024-12-16 =
* Fix: Gutenberg editor expanded disproportionately after adding support for `theme.json` ([#430](https://github.com/elementor/fugu-theme/issues/430))
...
* Fix: Minify JS files ([#419](https://github.com/elementor/fugu-theme/issues/419))
...
* Fix: Mobile Menu hamburger is not visible for logged-out users in some cases ([#369](https://github.com/elementor/fugu-theme/issues/369))
* Fix: Remove duplicate ID attributes in the header mobile menu
* Fix: Remove redundant table styles ([#311](https://github.com/elementor/fugu-theme/issues/311))
...
* Fix: Separate post tags in single post ([#304](https://github.com/elementor/fugu-theme/issues/304))
...
* Fix: Update pagination prev/next labels and positions ([#404](https://github.com/elementor/fugu-theme/issues/404))
...
* Tweak: Don't load `fugu-frontend.min.js` when disabling header & footer
...
* Tweak: Link from Elementor Site Settings to Fugu Theme Settings
...
* New: Option to unregister Fugu style.css
* New: Option to unregister Fugu theme.css
...
* Fix: Empty "Skip to content" href ([#276](https://github.com/elementor/fugu-theme/issues/276))
* Fix: Child themes using `fugu_elementor_body_open()` no longer working ([#278](https://github.com/elementor/fugu-theme/issues/278))
...
* Tweak: Add `fugu_elementor_enable_skip_link` filter to enable/disable the skip link
* Tweak: Add `fugu_elementor_skip_link_url` filter to change skip link URL
...
* Tweak: Delete deprecated `elementor_fugu_theme_load_textdomain` filter hook
* Tweak: Delete deprecated `elementor_fugu_theme_register_menus` filter hook
* Tweak: Delete deprecated `elementor_fugu_theme_add_theme_support` filter hook
* Tweak: Delete deprecated `elementor_fugu_theme_add_woocommerce_support` filter hook
* Tweak: Delete deprecated `elementor_fugu_theme_enqueue_style` filter hook
* Tweak: Delete deprecated `elementor_fugu_theme_register_elementor_locations` filter hook
...
* Tweak: Delete the `fugu_elementor_load_textdomain` filter hook

= 2.6.1 - 2022-07-11 =
* Tweak: Tables looks weird on dark backgrounds ([#126](https://github.com/elementor/fugu-theme/issues/126))
* Fix: Remove unnecessary PHP tags ([#213](https://github.com/elementor/fugu-theme/issues/213))

= 2.6.0 - 2022-07-10 =
* Tweak: Added `theme_support` for `script` and `style` to avoid validation warnings ([#184](https://github.com/elementor/fugu-theme/issues/184))
* Tweak: Sanitized content for allowed HTML tags in post title ([#118](https://github.com/elementor/fugu-theme/issues/118))
...
* Fix: Added output escaping in several places ([#194](https://github.com/elementor/fugu-theme/issues/194))
* Fix: Post Password Form Submit button alignment (Props [@romanbondar](https://github.com/romanbondar))
* Fix: Fatal error when kit doesn't exist or needs to be recreated ([#175](https://github.com/elementor/fugu-theme/issues/175))

= 2.5.0 - 2022-01-26 =
* Tweak: Added keyboard navigation to Fugu Elementor theme menus
* Tweak: Added Skip Links and `#content` for the main wrapper for better accessibility ([#133](https://github.com/elementor/fugu-theme/issues/133))
...
* Tweak: Remove comments in `style.min.css` output ([#179](https://github.com/elementor/fugu-theme/issues/179))
* Tweak: Promoted Fugu Elementor theme Header & Footer experiment status to Stable
* Tweak: Added compatibility for upcoming WordPress version 5.9

= 2.4.1 - 2021-07-07 =
* Fix: Fugu Elementor theme Header & Footer experiment should be inactive for existing sites
...
* Tweak: Added `input[type="url"]` to CSS reset rules ([#109](https://github.com/elementor/fugu-theme/issues/109))
...
= 2.2.1 - 2019-09-10 =
* Tweak: Added max width to `wp-caption` ([#91](https://github.com/elementor/fugu-theme/issues/91))
* Tweak: Added support of `wp_body_open`

= 2.2.0 - 2019-07-22 =
* Tweak: Added viewport content filter ([#49](https://github.com/elementor/fugu-theme/issues/49))
...
* Tweak: Remove header tags with `fugu_elementor_page_title` filter
...
* New: Added `fugu_elementor_page_title` filter for show\hide page title
* New: Added `fugu_elementor_enqueue_theme_style` filter for enqueue theme-specific style
...
= 2.0.5 - 2019-05-21 =
* New: Introducing [Fugu Theme Child](https://github.com/elementor/fugu-theme-child)
...
= 2.0.2 - 2019-05-13 =
* Tweak: Added `fugu_elementor_content_width` filter, as per WordPress best practice
...
* Tweak: Change functions.php methods names prefix from `fugu_elementor_theme_` to `fugu_elementor_`
...
* New: Added `elementor_fugu_theme_load_textdomain` filter for load theme's textdomain
* New: Added `elementor_fugu_theme_register_menus` filter for register the theme's default menu location
* New: Added `elementor_fugu_theme_add_theme_support` filter for register the various supported features
* New: Added `elementor_fugu_theme_add_woocommerce_support` filter for register woocommerce features, including product-gallery zoom, swipe & lightbox features
* New: Added `elementor_fugu_theme_enqueue_style` filter for enqueue style
* New: Added `elementor_fugu_theme_register_elementor_locations` filter for register elementor settings
