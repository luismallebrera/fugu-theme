...
class Settings_Controller {
	...
	const SETTINGS_FILTER_NAME = 'fugu-plus-theme/settings';
	const SETTINGS_PAGE_SLUG = 'fugu-elementor-settings';
	const SETTING_PREFIX = 'fugu_elementor_settings';
	const SETTINGS = [
		'DESCRIPTION_META_TAG' => '_description_meta_tag',
		'SKIP_LINK'            => '_skip_link',
		'HEADER_FOOTER'        => '_header_footer',
		'PAGE_TITLE'           => '_page_title',
		'FUGU_STYLE'          => '_fugu_style',
		'FUGU_THEME'          => '_fugu_theme',
	];
	...
	public function apply_settings( $settings_group = self::SETTING_PREFIX, $settings = self::SETTINGS ) {
		...
		$this->apply_setting(
			...
			function () {
				remove_action( 'wp_head', 'fugu_elementor_add_description_meta_tag' );
			}
		);
		...
		$this->apply_setting(
			...
			function () {
				add_filter( 'fugu_elementor_enable_skip_link', '__return_false' );
			}
		);
		...
		$this->apply_setting(
			...
			function () {
				add_filter( 'fugu_elementor_header_footer', '__return_false' );
			}
		);
		...
		$this->apply_setting(
			...
			function () {
				add_filter( 'fugu_elementor_page_title', '__return_false' );
			}
		);

		$this->apply_setting(
			$settings_group . $settings['FUGU_STYLE'],
			function () {
				add_filter( 'fugu_elementor_enqueue_style', '__return_false' );
			}
		);

		$this->apply_setting(
			$settings_group . $settings['FUGU_THEME'],
			function () {
				add_filter( 'fugu_elementor_enqueue_theme_style', '__return_false' );
			}
		);
	}
	...
	public function enqueue_fugu_plus_settings_scripts() {
	...
	public function __construct() {
		add_action( 'admin_enqueue_scripts', [ $this, 'enqueue_fugu_plus_settings_scripts' ] );
		add_action( 'fugu-plus-theme/admin-menu', [ $this, 'register_settings_page' ], 10, 1 );
	}
}
